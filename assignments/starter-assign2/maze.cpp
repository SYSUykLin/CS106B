...
void validatePath(Grid<bool>& maze, Stack<GridLocation> path) {
     ...
     */
    Set<GridLocation> pathElements;
    while(!path.isEmpty()){
        GridLocation curLocation = path.pop();

        if (pathElements.contains(curLocation))
        {
            error("Path contains a loop");
        }
        else
        {
            pathElements.add(curLocation);
        }

        if (path.isEmpty())
        {
            if (curLocation.col == 0 && curLocation.row == 0)
            {
                // 起点
                return;
            ...
            {   //已经是起点，但还有其他元素在path里面
                std::cout << curLocation << std::endl;
                std::cout << path;
                error("Path contains many elements");
            }
        }


        Set<GridLocation> previousLocations = generateValidMoves(maze, curLocation);
        if(path.isEmpty())
        {
           error("Path does not complete");
        }
        GridLocation previousStep = path.peek();
        if (previousLocations.contains(previousStep)){
           // 可以合法移动到
           continue;
        }
        else
        {
           error("Path is not a valid move");
        }
    }
}
